    ...
    *******************************************************************************
    ...
    def voxel_to_world(volume_origin, voxel_coords, voxel_size):
        ...
        for i in prange(voxel_coords.shape[0]):
            world_points[i] = np.transpose(volume_origin) + voxel_coords[i] * voxel_size
        return world_points
    ...
    def get_valid_points(self, depth_image, voxel_u, voxel_v, voxel_z):
        ...
        Args:
            ...
            voxel_u (numpy.array [v, ]): Voxel coordinate projected into camera coordinate, axis is u width
            voxel_v (numpy.array [v, ]): Voxel coordinate projected into camera coordinate, axis is v height
        ...
        v_points = np.empty_like(voxel_v, dtype=bool)
        for i in prange(len(voxel_u)):
            point_f = True
            if voxel_v[i] >= image_height or voxel_v[i] < 0 or voxel_u[i] >= image_width or voxel_u[i] < 0:
                point_f = False
            else:
    ...
    def integrate(self, color_image, depth_image, camera_intrinsics, camera_pose, observation_weight=1.):
        ...
        world_point = self.voxel_to_world(self._volume_origin, self._voxel_coords, self._voxel_size)
        camera_point = transform_point3s(transform_inverse(camera_pose), world_point)
        image_point = camera_to_image(camera_intrinsics, camera_point)
        # camera_uv = camera_point[:, :2]
        ...
        v_points = self.get_valid_points(depth_image, image_point[:, 0], image_point[:, 1], camera_point[:, 2])
        # print(v_points)
        ...
        valid_tsdf_x = self._voxel_coords[v_points][:, 0]
        valid_tsdf_y = self._voxel_coords[v_points][:, 1]
        valid_tsdf_z = self._voxel_coords[v_points][:, 2]
        ...
        v_pixels_x = image_point[v_points][:, 0]
        v_pixels_y = image_point[v_points][:, 1]
        v_camera_z = camera_point[v_points][:, 2]

        v_dis = np.empty_like(v_camera_z)
        for i in prange(len(v_dis)):
            dis = depth_image[v_pixels_y[i], v_pixels_x[i]] - v_camera_z[i]
            v_dis[i] = max(-1, min(1, dis/self._truncation_margin))
        ...
        self._tsdf_volume[valid_tsdf_x, valid_tsdf_y, valid_tsdf_z], weight_new = self.get_new_tsdf_and_weights(
                                        tsdf_old=self._tsdf_volume[valid_tsdf_x, valid_tsdf_y, valid_tsdf_z],
                                        margin_distance=v_dis,
        ...
        color_new = np.empty((len(v_pixels_x), 3))

        for i in range(len(v_pixels_x)):
            color_new[i] = color_image[v_pixels_y[i], v_pixels_x[i]]

        self._color_volume[valid_tsdf_x, valid_tsdf_y, valid_tsdf_z] = self.get_new_colors_with_weights(
                                                    self._color_volume[valid_tsdf_x, valid_tsdf_y, valid_tsdf_z],
                                                    color_new=color_new,
                                                    w_old=self._weight_volume[valid_tsdf_x, valid_tsdf_y, valid_tsdf_z],
                                                    w_new=weight_new,
                                                    observation_weight=observation_weight)
