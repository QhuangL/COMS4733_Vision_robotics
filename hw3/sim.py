...
class PyBulletSim:
    ...
    def move_tool(self, position, orientation, speed=0.01):
        ...
        target_joint_state = p.calculateInverseKinematics(bodyUniqueId=self.robot_body_id,
                                                          ...
                                                          targetPosition=position, targetOrientation=orientation,
    ...
    def get_grasp_position_angle(self, object_id):
        ...
        position, angle = p.getBasePositionAndOrientation(bodyUniqueId= object_id)
        grasp_angle = p.getEulerFromQuaternion(angle)

        # print("position:", position)
        # ==================================
        return position, grasp_angle[2]

    def robot_go_home(self, speed=0.3):
    ...
    def execute_grasp(self, grasp_position, grasp_angle):
        ...
        self.move_tool(position=pre_grasp_position_over_bin, orientation=gripper_orientation, speed=0.03)
        # 3. Move gripper to pre_grasp_position_over_object
        self.move_tool(position=pre_grasp_position_over_object, orientation=gripper_orientation, speed=0.03)
        # 4. Move gripper to grasp_position
        self.move_tool(position=grasp_position, orientation=gripper_orientation, speed=0.03)
        # 5. Close gripper
        self.close_gripper()
        # 6. Move gripper to post_grasp_position
        self.move_tool(position=post_grasp_position, orientation=gripper_orientation, speed=0.03)
    ...
    def check_collision(self, q):
        ...
        for obstacle_id in self.obstacles:
            closest_points = p.getClosestPoints(
                self.robot_body_id, obstacle_id, 0.2)
