...
def align_pts(pts_a, pts_b, max_iterations=10000, threshold=1e-08):
    """
    In:
    ...
    """
    # TODO
    pro_matrix, pro_image, pro_cost = trimesh.registration.procrustes(pts_a, pts_b,  scale=False, reflection=False)
    trans_matrix, trans_image, trans_cost = trimesh.registration.icp(pts_a, pts_b, initial = pro_matrix, threshold = threshold, max_iterations = max_iterations, scale=False, reflection=False)
...
def main():
    ...
    for scene_id in range(5):
        ...
        mask = image.read_mask(dataset_dir+'pred/'+str(scene_id)+'_pred.png')
        view_matrix = np.load(dataset_dir+'view_matrix/'+str(scene_id)+'.npy')
        pred_pose = estimate_pose(depth, mask, my_camera, view_matrix)
