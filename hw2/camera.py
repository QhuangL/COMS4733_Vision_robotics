...
class Camera(object):
    ...
    def compute_camera_matrix(self):
        ...
        aspect = self.image_size[1]/self.image_size[0]  # image width/height
        # intrinsic matrix = [fx  s  cx         fx=focal.length = fy
        #                     0   fy cy         s = 0 if the image axis is perpendicular
        #                     0   0   1]        cx, cy is optical center(principle point) in pixels
        projection_matrix = p.computeProjectionMatrixFOV(fov=self.fov_height,aspect=aspect,nearVal=self.near,farVal=self.far)  # TODO: compute projection_matrix from camera parameters
